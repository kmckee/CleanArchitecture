...
task Test {
	$projects |
		ForEach-Object {
			$nunitPath = Get-PackagePath "NUnit.ConsoleRunner" $($_.Directory)
			if($nunitPath -eq $null){
				return
			} 	
			$nunitRunner = "$nunitPath\tools\nunit3-console.exe"

			exec { & $nunitRunner $absoluteOutputDirectory\$($_.Name)\$($_.Name).dll `
						"--result=$absoluteOutputDirectory\nunit_$($_.Name).xml`";format=nunit3" }
		}	
}
